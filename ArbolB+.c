...
int cargar_datos(page **raiz);
datos *crear_datos(int d,datos *p);  //Aloja espacio de memoria para los datos a almacenar
clave *crear_clave(int d,datos *p); //Aloja espacio de memoria para almacenar las claves a alojar en una pagina
clave ** clave_centro(clave **raiz, int c); //Obtiene la clave que se encuentre al centro de una pagina
page *crear_pagina(int d,datos *p); // Aloja espacio de memoria para cada pagina
...
void insertar_claves_en_pag(clave **raiz_p,int d,datos *p); // Interta claves en una pagina
...
int insercion(page **raiz,int id,datos *datos_p); // Operacion general de insercion
...
int cargar_datos(page **raiz)
{
	...
	filee=fopen("Paciente.bin","rb+");
	 if(filee==NULL){
		printf("No se puede abrir el archivo\n");
		//return 0;
	}else{
		datos paciente;
		fread(&paciente,sizeof(datos),1,filee);
		insercion(raiz,a[0],&paciente);
		while(!feof(filee))
		{
			i++;
			insercion(raiz,a[i],&paciente);	
			fread(&paciente,sizeof(datos),1,filee);
		}
		fclose(filee);
	}
...
datos *crear_datos(int d,datos *p)  //Aloja espacio de memoria para los datos a almacenar
{
	...
	{
		strcpy(newe->nombre_p,p->nombre_p);
		strcpy(newe->apellido_pp,p->apellido_pp);
		strcpy(newe->apellido_pm,p->apellido_pm);
		newe->edad=p->edad;
		strcpy(newe->piso,p->piso);
		newe->cama=p->cama;
	}
	return newe;
}
...
clave *crear_clave(int d,datos *p) //Aloja espacio de memoria para almacenar las claves a alojar en una pagina
{
	...
	if(!newe)
		return NULL;
	newe->clv=d;
	newe->datos_alm=crear_datos(d,p);
	newe->abajo=NULL;
	newe->sig=NULL;
	newe->ant=NULL;
	return newe;
...
int insercion(page **raiz,int id,datos *p) // Operacion general de insercion
{
	if(!(*raiz))
	{
		(*raiz)=crear_pagina(id,p);
		if(raiz)
			return 1;
	}
	int bnd2;
	...
	{
		bnd2=insercion(&(*raiz)->ant,id,p);     //inicial de la pagina
	...
	if(aux_mayor && ((*aux_mayor)->abajo))
		{
			bnd2=insercion(&(*aux_mayor)->abajo,id,p); //Inserta sin llevarse arriba a otra clave..
	...
	if(bnd==0)
		insertar_claves_en_pag(&(*raiz)->inicio,id,p);
